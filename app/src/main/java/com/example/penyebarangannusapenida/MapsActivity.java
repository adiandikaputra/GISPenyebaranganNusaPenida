...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    private void getlokasi(){
        String url="https://penyebranganpenida.000webhostapp.com/penyebrangan.php";
                        ...
                        for (int i = 0; i < jumdata; i++) {
                            try {
                                ...
                                mMap.addMarker(new MarkerOptions()
                                        .position(latLng[i])
                                        .snippet(alamat[i])
                                        .icon(BitmapDescriptorFactory.fromResource(R.drawable.bot))
                                        .title(nama[i]));
                               ...
                               for (int i=0;i<jumdata;i++){
                                   if(marker.getTitle().equals(nama[i])) {
                                       if (MarkerD[i]) {
                                           ...
                                           Toast pesan = Toast.makeText(MapsActivity.this, "Silikan Klik Sekali Lagi Untuk Detail", Toast.LENGTH_LONG);
